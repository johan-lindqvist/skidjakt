...
function buildQueryString(query: DealQuery): string {
  ...
  if (query.destinations?.length)
    query.destinations.forEach((d) => params.append('destinations', d));
...
export const api = {
  getDeals: (query: DealQuery = {}): Promise<PagedResult<Deal>> =>
    fetchJson(`${API_BASE}/deals?${buildQueryString(query)}`),

  getDeal: (id: number): Promise<Deal> => fetchJson(`${API_BASE}/deals/${id}`),

  getFilters: (): Promise<DealFilterOptions> =>
    fetchJson(`${API_BASE}/deals/filters`),
