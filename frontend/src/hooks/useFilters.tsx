...
function parseFiltersFromUrl(): DealQuery {
  ...
  return {
    ...
    agencies: params.getAll('agencies').length
      ? params.getAll('agencies')
      : undefined,
    countries: params.getAll('countries').length
      ? params.getAll('countries')
      : undefined,
    transportTypes: params.getAll('transportTypes').length
      ? params.getAll('transportTypes')
      : undefined,
...
function syncFiltersToUrl(query: DealQuery) {
  ...
  if (query.agencies?.length)
    query.agencies.forEach((a) => params.append('agencies', a));
  if (query.countries?.length)
    query.countries.forEach((c) => params.append('countries', c));
  if (query.transportTypes?.length)
    query.transportTypes.forEach((t) => params.append('transportTypes', t));
...
export function FilterProvider({ children }: { children: ReactNode }) {
  ...
  const toggleArray = useCallback(
    (current: string[] | undefined, value: string): string[] | undefined => {
  ...
  const setPriceRange = useCallback(
    (min?: number, max?: number) =>
      setQuery((q) => ({ ...q, minPrice: min, maxPrice: max, page: 1 })),
    [],
  );
  const setDateRange = useCallback(
    (from?: string, to?: string) =>
      setQuery((q) => ({ ...q, fromDate: from, toDate: to, page: 1 })),
    [],
  );
  const setSortBy = useCallback(
    (sort: string) => setQuery((q) => ({ ...q, sortBy: sort, page: 1 })),
    [],
  );
