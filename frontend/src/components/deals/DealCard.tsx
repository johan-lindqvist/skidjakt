...
export function DealCard({ deal }: DealCardProps) {
  const hasDiscount =
    deal.originalPrice && deal.originalPrice > deal.pricePerPerson;
  const discountPercent = hasDiscount
    ? Math.round(
        ((deal.originalPrice! - deal.pricePerPerson) / deal.originalPrice!) *
          100,
      )
    : 0;
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          {deal.includesMeals && (
            <span
              className="flex items-center gap-1 text-xs text-slate-400"
              title="Måltider ingår"
            >
              <UtensilsCrossed className="h-3.5 w-3.5" />{' '}
              {deal.mealDescription || 'Måltider'}
            </span>
          )}
          {deal.includesTransfer && (
            <span
              className="flex items-center gap-1 text-xs text-slate-400"
              title="Transfer ingår"
            >
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <span className="text-2xl font-bold text-sky-400">
              {deal.pricePerPerson.toLocaleString('sv-SE')} kr
            </span>
