...
api.MapGet(
    ...
    async (IDealRepository repo, CancellationToken ct) =>
        Results.Ok(await repo.GetFilterOptionsAsync(ct))
);

// GET /api/deals/stats
api.MapGet(
    "/deals/stats",
    async (IDealRepository repo, CancellationToken ct) => Results.Ok(await repo.GetStatsAsync(ct))
);
        ...
        _ = Task.Run(
            async () =>
            {
                try
                {
                    ...
                    var scrapers = scope.ServiceProvider.GetRequiredService<
                        IEnumerable<IDealScraper>
                    >();
