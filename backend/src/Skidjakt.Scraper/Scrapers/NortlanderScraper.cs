...
public class NortlanderScraper : IDealScraper
{
    ...
    public NortlanderScraper(
        IHttpClientFactory httpClientFactory,
        ILogger<NortlanderScraper> logger
    )
    {
            ...
            foreach (var url in urls)
            {
                try
                {
                    ...
                    var dealElements = document.QuerySelectorAll(
                        "[class*='trip'], [class*='deal'], [class*='offer'], [class*='package'], .product-card"
                    );
    ...
    private Deal? ParseDealElement(IElement element, string baseUrl)
    {
        var destination = element
            .QuerySelector("h2, h3, h4, .title, .destination, [class*='title']")
            ?.TextContent?.Trim();
        ...
        var durationText = element
            .QuerySelector("[class*='duration'], [class*='night'], .nights")
            ?.TextContent?.Trim();
        ...
        var originalPriceText = element
            .QuerySelector("del, s, [class*='original'], [class*='was']")
            ?.TextContent?.Trim();
