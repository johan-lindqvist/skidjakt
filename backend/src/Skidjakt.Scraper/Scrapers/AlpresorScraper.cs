...
public class AlpresorScraper : IDealScraper
{
	...
	public async Task<IReadOnlyList<Deal>> ScrapeDealsAsync(CancellationToken ct)
	{
		...
		try
		{
			...
			var response = await client.GetStringAsync("https://www.alpresor.se/sista-minuten/", ct);
			...
			var dealElements = document.QuerySelectorAll("[class*='trip-card'], [class*='deal'], [class*='package-item'], .search-result");
	...
	{
		var destination = element.QuerySelector("h2, h3, .destination, .trip-title")?.TextContent?.Trim();
		...
		var country = element.QuerySelector(".country, .region, [class*='country']")?.TextContent?.Trim() ?? "Okänt";

		var durationText = element.QuerySelector("[class*='duration'], [class*='nights'], .nights")?.TextContent?.Trim();
		var durationNights = ParseDuration(durationText);

		var dateText = element.QuerySelector("[class*='date'], .departure-date, .dates")?.TextContent?.Trim();

		var originalPriceText = element.QuerySelector("[class*='original'], del, .was-price, s")?.TextContent?.Trim();
		TryParsePrice(originalPriceText, out var originalPrice);

		var accommodation = element.QuerySelector("[class*='hotel'], [class*='accommodation'], .hotel-name")?.TextContent?.Trim();
		...
		return new Deal
		{
			...
			SourceUrl = !string.IsNullOrEmpty(link) ? link : "https://www.alpresor.se/sista-minuten/",
