...
public class SlopestarScraper : IDealScraper
{
        ...
        try
        {
            ...
            if (deals.Count == 0)
            {
                _logger.LogInformation(
                    "No deals found via HTTP for Slopestar, site may require JS rendering"
                );
    ...
    private async Task<List<Deal>> ScrapeWithHttpAsync(CancellationToken ct)
    {
        ...
        var response = await client.GetStringAsync(
            "https://www.slopestar.se/sista-minuten-skidresor",
            ct
        );
        ...
        var dealElements = document.QuerySelectorAll(
            "[class*='trip'], [class*='deal'], [class*='product'], [class*='card'], .search-result"
        );
    ...
    private Deal? ParseDealElement(IElement element)
    {
        var destination = element
            .QuerySelector("h2, h3, h4, .destination, .title, [class*='title']")
            ?.TextContent?.Trim();
        ...
        var country =
            element
                .QuerySelector(".country, [class*='country'], [class*='region']")
                ?.TextContent?.Trim()
            ?? "Okänt";

        var durationText = element
            .QuerySelector("[class*='duration'], [class*='night']")
            ?.TextContent?.Trim();
        ...
        var originalPriceText = element
            .QuerySelector("del, s, [class*='original']")
            ?.TextContent?.Trim();
