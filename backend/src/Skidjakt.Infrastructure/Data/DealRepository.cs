...
public class DealRepository : IDealRepository
{
    ...
    )
    {
        ...
        {
            q = q.Where(d =>
                d.TransportType != null && query.TransportTypes.Contains(d.TransportType)
            );
        }
    ...
    public async Task<DealFilterOptions> GetFilterOptionsAsync(CancellationToken ct = default)
    {
        ...
        var agencies = await activeDeals
            .Select(d => d.Agency)
            .Distinct()
            .OrderBy(a => a)
            .ToListAsync(ct);

        var countries = await activeDeals
            .Select(d => d.Country)
            .Distinct()
            .OrderBy(c => c)
            .ToListAsync(ct);

        var destinations = await activeDeals
        ...
        var minPrice = await activeDeals.AnyAsync(ct)
            ? await activeDeals.MinAsync(d => d.PricePerPerson, ct)
            : 0;

        var maxPrice = await activeDeals.AnyAsync(ct)
            ? await activeDeals.MaxAsync(d => d.PricePerPerson, ct)
            : 0;

        return new DealFilterOptions(
            agencies,
            countries,
            destinations,
            transportTypes,
    ...
    public async Task<DealStats> GetStatsAsync(CancellationToken ct = default)
    {
        ...
        var averagePrice =
            totalDeals > 0 ? (int)await activeDeals.AverageAsync(d => d.PricePerPerson, ct) : 0;

        var lowestPrice =
            totalDeals > 0 ? await activeDeals.MinAsync(d => d.PricePerPerson, ct) : 0;

        var lastUpdated =
            totalDeals > 0 ? await activeDeals.MaxAsync(d => (DateTime?)d.LastSeen, ct) : null;

        return new DealStats(totalDeals, dealsPerAgency, averagePrice, lowestPrice, lastUpdated);
    }

    public async Task UpsertDealsAsync(
        string agency,
    ...
    )
    {
        ...
        var existingDeals = await _db
            .Deals.Where(d => d.Agency == agency)
